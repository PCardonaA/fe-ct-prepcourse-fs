...
function numeroMasGrande(arrayOfNums) {
   ...
   for (var i = 0;i < arrayOfNums.length;i++){
      ...
      } else if (arrayOfNums[i]> max){
          var max = arrayOfNums[i];
       }
     }
   return(max);
}
...
function empiezaConNueve(num) {
   ...
   var numerosSeparados = stringNum.split('');
   if (numerosSeparados[0]=== '9'){
     return true;
   }else{
     return false;
   }
...
function todosIguales(array) {
   ...
   // Tu código:
   iguales = array[0];
   var cumplenCondicion = array.every((num)=> {
      return num === iguales;
   });
...
function mesesDelAño(array) {
   ...
   if(array.includes('Enero') & array.includes('Marzo') & array.includes('Noviembre')){
      seleccionMeses.unshift('Marzo','Noviembre','Enero');
      return(seleccionMeses); 
   } else {
